...
function New-IsolatedProject {
    ...
    New-Item -ItemType Directory -Path $projectPath, (Join-Path $projectPath 'scripts'), (Join-Path $projectPath 'docs'), (Join-Path $projectPath 'versions') -Force | Out-Null
    ...
    Copy-Item -LiteralPath $sourcePath -Destination (Join-Path $projectPath 'docs\index.html')
    Copy-Item -LiteralPath $sourcePath -Destination (Join-Path $projectPath 'versions\scoring-calculator-v1.1.html')
    ...
    foreach ($name in 'new-release.ps1', 'verify-project.ps1', 'backup-repo.ps1') {
        Copy-Item -LiteralPath (Join-Path $RepositoryRoot (Join-Path 'scripts' $name)) -Destination (Join-Path $projectPath (Join-Path 'scripts' $name))
    }
...
function Add-PrivacyHistory {
    ...
    $secretPath = Join-Path $ProjectPath '.workbuddy\history-secret.txt'
    ...
    try {
        ...
        git add -f -- '.workbuddy/history-secret.txt'
...
'@
    ...
    if ($env:IJRU_RUN_DEFAULT_BACKUP_TEST -eq '1') {
